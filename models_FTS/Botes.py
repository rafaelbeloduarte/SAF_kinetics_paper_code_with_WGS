...
def Botes():
    ...
    param_dict = {
        ...
        'H_ads': 5e4, # J/mol
        'A_HCs': ( 4.7e-5 / 5.12 / 1000 ) / ( 1e6**1.25 * e**( - 1e5 / (8.314*500) ) ),
        'E_HCs': 1e5, # J/mol
    }
    ...
    for parafin in n_parafins:
        ...
        def rate_n_para(param_dict, T, P, x, F, K, eq_distance, R, data, n = n):
            ...
            r = y_n * upsilon_n * k_HCs * P_H2**0.75 * P_CO**0.5 / (1 + K_ads * P_CO**0.5)**2
            return r
    ...
    for olefin in n_olefins:
        ...
        def rate_n_ole(param_dict, T, P, x, F, K, eq_distance, R, data, n = n):
            ...
            K_ads = k_ads * e**(-H_ads / (R * T))
            k_HCs = A_HCs * e**(-E_HCs / (R * T))
            r = y_n * (1 - upsilon_n) * k_HCs * P_H2**0.75 * P_CO**0.5 / (1 + K_ads * P_CO**0.5)**2
            return r
