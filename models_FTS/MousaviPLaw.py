...
def MousaviPLaw():
    model_name = 'MousaviPLaw'
    param_dict = {
        'k_ads': 0.1 / ( 1e6**0.5 * e**( - 5e4 / (8.314*500) ) ),
        'H_ads': 5e4, # J/mol
        'A_HCs': ( 4.7e-5 / 5.12 / 1000 ) / ( 1e6**1.75 * e**( - 1e5 / (8.314*500) ) ),
        'E_HCs': 1e5, # J/mol
        'a'    : 1,
        'b'    : 1,
        'c'    : 1,
    ...
    for parafin in n_parafins:
        ...
        def rate_n_para(param_dict, T, P, x, F, K, eq_distance, R, data, n = n):
            ...
            r = y_n * upsilon_n * k_HCs * P_H2**a * P_CO**b / (1 + K_ads*P_CO**c)**2
            return r
    ...
    for olefin in n_olefins:
        ...
        def rate_n_ole(param_dict, T, P, x, F, K, eq_distance, R, data, n = n):
            ...
            K_ads = k_ads * e**(-H_ads / (R * T))
            k_HCs = A_HCs * e**(-E_HCs / (R * T))
            r = y_n * (1 - upsilon_n) * k_HCs * P_H2**a * P_CO**b / (1 + K_ads*P_CO**c)**2
            return r
