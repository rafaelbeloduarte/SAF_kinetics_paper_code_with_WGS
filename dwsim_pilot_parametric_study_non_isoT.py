...
def simulate_point(index, row, dwsim_path, sim_path):
    ...
    # to set the effectiveness factor
    for item in Flowsheet.Scripts.GetEnumerator():
        item.Value.ScriptText = item.Value.ScriptText.replace("thiele_220C = 4", 
                                                              f"thiele_220C = {str(row['phi_220C'])}")

    # Set parameters
    H2.SetMolarFlow(row['F_H2_in'])
    CO.SetMolarFlow(row['F_CO_in'])
    compressor.POut = row['P_Pa']
    PFR_1.OutletTemperature = row['T_out_K']
